...
namespace ThePeacenet.Gui.Controls
{
    public sealed class TextBox : Control
    {
        ...
        public override Size2 GetContentSize(IGuiContext context)
        {
            ...
            var stringSize = (Size2)font.MeasureString(Text ?? string.Empty);
            return new Size2(stringSize.Width,
                stringSize.Height < font.Spacing ? font.Spacing : stringSize.Height);
        }
        ...
        private int FindNearestGlyphIndex(IGuiContext context, Point position)
        {
            ...
            foreach (char c in textInfo.Text)
            {
                var bounds = font.GetTextBounds(position.ToVector2(), c.ToString());
                var fontRegionWidth = bounds.Width;
                var glyphMiddle = (int)(bounds.Left + fontRegionWidth * 0.5f);
